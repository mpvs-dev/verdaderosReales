...
function Toggle({ enabled, onClick, color = "purple" }) {
  ...
  return (
    <div onClick={onClick} style={{
      width: 44, height: 24, borderRadius: 999, background: bg,
...
const PILL_STYLE = {
  purple: { background: "rgba(109,40,217,0.3)",  border: "1.5px solid rgba(139,92,246,0.5)", color: "#C4B5FD" },
  gold:   { background: "rgba(245,158,11,0.2)",  border: "1.5px solid rgba(245,158,11,0.45)", color: "#FCD34D" },
  red:    { background: "rgba(239,68,68,0.2)",   border: "1.5px solid rgba(239,68,68,0.45)",  color: "#FCA5A5" },
  green:  { background: "rgba(16,185,129,0.15)", border: "1.5px solid rgba(16,185,129,0.35)", color: "#6EE7B7" },
};
...
function CategorySelector({ selected, onChange }) {
  ...
  return (
    <div>
          ...
          return (
            <div
              ...
              style={{
                ...
                background: active ? "rgba(109,40,217,0.2)" : "rgba(255,255,255,0.04)",
            ...
            >
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.3)", lineHeight: 1.3, marginTop: 1 }} className="truncate">
                  {cat.description}
                </div>
              </div>
            </div>
      ...
      {selected.length === 0 && (
        <p style={{ fontSize: 11, color: "#FCA5A5", fontWeight: 700, marginTop: 8 }}>
...
export default function GameConfigModal({ config, onClose, onSave }) {
  ...
  const modeOptions = [
    { value: "generic", label: t("config.modeGenericLabel"), desc: t("config.modeGenericDesc") },
    { value: "custom",  label: t("config.modeCustomLabel"),  desc: t("config.modeCustomDesc")  },
  ];
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 18, fontWeight: 800, color: "#fff" }}>{t("config.title")}</span>
          </div>
        ...
        {/* Mode selector */}
        <div>
          <div className="t-label" style={{ marginBottom: 8 }}>{t("config.gameMode")}</div>
            ...
            {modeOptions.map((m) => {
              ...
              return (
                <div key={m.value} onClick={() => setLocal((p) => ({ ...p, mode: m.value }))}
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 800, color: active ? "#fff" : "rgba(255,255,255,0.7)" }}>
                      {m.value === "generic" ? "Genéricas" : "Custom"}
                    </div>
                    <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.38)", marginTop: 2, lineHeight: 1.3 }}>
                      {m.desc}
                    </div>
                  </div>
                </div>
        ...
        <div>
          ...
          <input type="range" min={2} max={20} step={1} value={local.rounds}
        ...
        {!local.customPointsEnabled && (
          <div>
            ...
            <input type="range" min={1} max={10} step={1} value={local.pointsPerAnswer}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.35)", marginTop: 4 }}>
              ...
              <span style={{ color: "rgba(255,255,255,0.45)" }}>
                {local.pointsPerAnswer > 1
                  ? t("config.pointsDescPlural", { points: local.pointsPerAnswer })
        ...
        {isCustom && (
          <div onClick={() => setLocal((p) => ({ ...p, customPointsEnabled: !p.customPointsEnabled }))}
            style={{
            ...
            <Toggle enabled={local.customPointsEnabled} onClick={() => setLocal((p) => ({ ...p, customPointsEnabled: !p.customPointsEnabled }))} />
          </div>
        )}

        {/* Penalty toggle */}
        <div onClick={() => setLocal((p) => ({ ...p, penaltyEnabled: !p.penaltyEnabled }))}
          style={{
            display: "flex", alignItems: "center", gap: 12,
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.38)", marginTop: 1 }}>
              {local.customPointsEnabled
                ...
                : local.pointsPerAnswer > 1
                  ? t("config.penaltyDescFixedPlural", { points: local.pointsPerAnswer })
                  : t("config.penaltyDescFixed",       { points: local.pointsPerAnswer })}
            </div>
          </div>
          <Toggle enabled={local.penaltyEnabled} color="red" onClick={() => setLocal((p) => ({ ...p, penaltyEnabled: !p.penaltyEnabled }))} />
        </div>
        ...
        <div style={{
          ...
          <div className="pill pill-purple">
            {local.mode === "custom" ? t("config.summaryCustom") : t("config.summaryGeneric")}
          </div>
          {/* Categorías activas en modo genérico */}
          {!isCustom && (local.categories ?? ["genericas"]).map((catId) => {
            const cat = QUESTION_CATEGORIES.find((c) => c.id === catId);
            return cat ? (
              <div key={catId} className="pill pill-gold">{cat.label}</div>
            ) : null;
          })}
          <div className="pill pill-gold">{t("config.summaryRounds", { rounds: local.rounds })}</div>
          {!local.customPointsEnabled && (
            <div className="pill pill-purple">{t("config.summaryPointsHit", { points: local.pointsPerAnswer })}</div>
          )}
          {local.penaltyEnabled && !local.customPointsEnabled && (
            <div className="pill pill-red">{t("config.summaryPointsMiss", { points: local.pointsPerAnswer })}</div>
          )}
          {local.customPointsEnabled && <div className="pill pill-purple">{t("config.summaryCustomPoints")}</div>}
          {local.penaltyEnabled && local.customPointsEnabled && <div className="pill pill-red">{t("config.summaryPenaltyActive")}</div>}
        </div>
