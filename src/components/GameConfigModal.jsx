...
function Toggle({ enabled, onClick, color = "purple" }) {
  const bg = enabled ? (color === "red" ? "#EF4444" : "#7C3AED") : "rgba(255,255,255,0.12)";
  return (
...
export default function GameConfigModal({ config, onClose, onSave }) {
  ...
  const [local, setLocal] = useState({ penaltyEnabled: false, customPointsEnabled: false, ...config });
  ...
  return (
    <div style={{
      ...
      <div style={{
        width: "100%", maxWidth: 440, background: "#16082e",
        border: "1.5px solid rgba(139,92,246,0.35)", borderRadius: 24, padding: 22,
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              width: 36, height: 36, borderRadius: 10, background: "rgba(109,40,217,0.35)",
              border: "1.5px solid rgba(139,92,246,0.4)", display: "flex", alignItems: "center", justifyContent: "center",
            }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {modeOptions.map((m) => {
              ...
              return (
                <div key={m.value} onClick={() => setLocal((p) => ({ ...p, mode: m.value }))} style={{
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 800, color: active ? "#fff" : "rgba(255,255,255,0.7)" }}>{m.label}</div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.4)", marginTop: 1 }}>{m.desc}</div>
                  </div>
                </div>
              );
            })}
        ...
        <div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.35)", marginTop: 4 }}>
            ...
            <span style={{ color: "rgba(255,255,255,0.45)" }}>
              {isCustom ? t("config.roundsDescCustom", { rounds: local.rounds }) : t("config.roundsDescGeneric", { rounds: local.rounds })}
            </span>
        ...
        {isCustom && (
          <div onClick={() => setLocal((p) => ({ ...p, customPointsEnabled: !p.customPointsEnabled }))} style={{
            display: "flex", alignItems: "center", gap: 12,
            ...
            <Toggle enabled={local.customPointsEnabled}
              onClick={() => setLocal((p) => ({ ...p, customPointsEnabled: !p.customPointsEnabled }))} />
          </div>
        )}

        {/* Penalty toggle */}
        <div onClick={() => setLocal((p) => ({ ...p, penaltyEnabled: !p.penaltyEnabled }))} style={{
          display: "flex", alignItems: "center", gap: 12,
        ...
        <div style={{
          ...
          borderRadius: 12, padding: "10px 14px", display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center",
        }}>
          <div className="pill pill-purple">{local.mode === "custom" ? t("config.summaryCustom") : t("config.summaryGeneric")}</div>
          ...
          {local.customPointsEnabled && (
            <div className="pill pill-purple">{t("config.summaryCustomPoints")}</div>
          )}
          {local.penaltyEnabled && local.customPointsEnabled && (
            <div className="pill pill-red">{t("config.summaryPenaltyActive")}</div>
          )}
        </div>
