        ...
        <div key={p.id} className="score-chip">
          ...
          <span
            style={{
              ...
              height: 7,
              borderRadius: "50%",
              background: DOT_COLOR[getState(p.id)] ?? DOT_COLOR.pending,
              flexShrink: 0,
            }}
          />
          <span
            style={{ fontSize: 12, fontWeight: 800, color: "var(--c-gold)" }}
          >
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
        {question.options?.map((opt, i) => (
          <button
            key={opt}
            className="answer-btn anim-slide"
            disabled={sending}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      <div style={{ position: "relative" }}>
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") submit(text.trim());
          }}
  ...
  const cfg =
    {
      [S.CORRECT]: {
        ...
        icon: "✅",
        title: t("playing.correct"),
        titleColor: "#6EE7B7",
      },
      [S.INCORRECT]: {
        cls: "feedback-incorrect",
        icon: "❌",
        title: t("playing.incorrect"),
        titleColor: "#FCA5A5",
      },
      [S.ANSWERED]: {
        cls: "feedback-waiting",
        icon: "⏳",
        title: t("playing.answerSent"),
        titleColor: "#93C5FD",
      },
    }[state] ?? {};
  ...
  return (
    <div className={`feedback ${cfg.cls} anim-pop`}>
      ...
      {show && state === S.CORRECT && (
        ...
        >
          {t("playing.pointsHit", { points: pts })}
        </div>
      )}
      {show && state === S.INCORRECT && pen && (
        <div
          ...
          style={{ display: "inline-flex", marginBottom: 6 }}
        >
          {t("playing.pointsMiss", { points: pnt })}
        </div>
      )}
      {show && state === S.ANSWERED && (
        ...
        >
          <div className="pill pill-purple">
            {t("playing.pointsIfHit", { points: pts })}
          </div>
          {pen && (
            <div className="pill pill-red">
              {t("playing.pointsIfMiss", { points: pnt })}
            </div>
          )}
        </div>
      )}
...
function ValuePills({ question, config, t }) {
  ...
  return (
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}>
      <div className="pill pill-purple">
        {t("playing.pointsIfHit", { points: question?.points ?? gp })}
      </div>
      {pen && (
        <div className="pill pill-red">
          {t("playing.pointsIfMiss", { points: question?.penalty ?? gp })}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      <div className="t-label" style={{ marginBottom: 8 }}>
        {t("playing.myPreviousAnswers")}
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
        {entries.map((e, i) => {
          const q = questions?.find(
            (q) => String(q.id) === String(e.questionId),
          );
          return (
            <div
              key={i}
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div
                  className="truncate"
                  style={{ fontSize: 13, color: "#fff", fontWeight: 800 }}
                >
                  {e.answer}
                </div>
              </div>
...
function KingAnswerSection({
  ...
  const rec = currentRoom.answers?.[kingIdForState]?.find(
...
export default function PlayingScreen({
  ...
  function myState() {
    const rec = currentRoom.answers?.[me?.id]?.find(
      (h) => String(h.questionId) === String(qId),
    );
    if (rec) return rec.isCorrect ? S.CORRECT : S.INCORRECT;
  ...
  const myCurrentAnswer = currentRoom.currentAnswers?.find(
    (a) =>
      a.aspirantName === playerName && String(a.questionId) === String(qId),
  );
  ...
  const alreadyValidated = allPlayers.filter((p) =>
    currentRoom.answers?.[p.id]?.some(
      (h) => String(h.questionId) === String(qId),
    ),
  ).length;
  const kingId = currentRoom.king?.id;
  const pendingAnswers = (currentRoom.currentAnswers ?? []).filter(
    (a) => String(a.aspirantId) !== String(kingId),
  ).length;
      ...
      <ScreenWrapper withBg onExit={resetGame}>
        ...
        {allAnswered ? (
          ...
          >
            ...
            <span style={{ fontSize: 13, fontWeight: 800, color: "#6EE7B7" }}>
              {t("playing.allAnswered")}
            </span>
          </div>
        ) : (
          <div className="glass">
            <span
              style={{ fontSize: 13, fontWeight: 700, color: "var(--c-w60)" }}
            >
              {t("playing.waitingAnswers", {
        ...
        {q?.type !== "text" ? (
          <>
            ...
            String(currentRoom.kingAnswer.questionId) === String(qId) ? (
              <div
                className="glass"
              ...
              >
                ...
                <div>
                  ...
                  >
                    {t("playing.kingAnswerRef")}:{" "}
                    <strong style={{ color: "#fff" }}>
                      {currentRoom.kingAnswer.answer}
                    </strong>
                  </p>
                </div>
              </div>
                  ...
                  <div style={{ display: "flex", gap: 7, flexShrink: 0 }}>
                    <button
                      ...
                      style={{
                        ...
                        padding: "9px 11px",
                        cursor: "pointer",
                        boxShadow: "0 4px 0 var(--c-success-dark)",
                        display: "flex",
                        alignItems: "center",
                      }}
                    >
                    ...
                    <button
                      onClick={() => handleValidate(ans.aspirantId, false)}
                      disabled={validating.has(ans.aspirantId)}
                      style={{
  ...
  return (
    <ScreenWrapper withBg onExit={resetGame}>
      ...
      <Scoreboard
        currentRoom={currentRoom}
        getState={(id) => (id === me?.id ? state : stateOf(id))}
        avatarMap={avatarMap}
      />
      <div className="question-box">
        <div className="question-over">
          {t("playing.aboutKing", { kingName: currentRoom.king?.name })}
        </div>
        <div className="question-text">{q?.text}</div>
        <ValuePills question={q} config={currentRoom.config} t={t} />
      </div>
