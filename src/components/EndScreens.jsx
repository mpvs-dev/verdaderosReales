...
import { formatDuration, getEveryone } from "../utils/room.js";
  ...
  return (
    <ScreenWrapper withBg onExit={resetGame}>
      ...
      <div style={{ textAlign: "center" }}>
        ...
        <h1 className="t-display" style={{ fontSize: 30, color: "#fff", marginBottom: 4 }}>¡Juego Terminado!</h1>
        {duration && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
            <Clock size={13} color="var(--c-w45)" />
            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--c-w45)" }}>
              Duración: <span style={{ color: "var(--c-w60)" }}>{duration}</span>
            </span>
          </div>
        )}
      </div>
      ...
      {winner && (
        <div className="anim-pop glass-gold" style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="truncate" style={{ fontSize: 17, fontWeight: 800, color: "#fff" }}>
              {winner.player?.name}
            </div>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--c-w45)", marginTop: 2 }}>
              Mejor conoce al Líder
            </div>
          </div>
          <div className="t-display" style={{ fontSize: 28, color: "var(--c-gold)", flexShrink: 0 }}>
            {winner.score} pts
          </div>
        </div>
      )}
        ...
        {sorted.map((entry, idx) => (
          <div key={entry.player?.id} className="glass">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span className="truncate" style={{ flex: 1, fontSize: 14, fontWeight: 800, color: "#fff" }}>
                {entry.player?.name}
              </span>
              <span className="t-display" style={{ fontSize: 20, color: "var(--c-gold)", flexShrink: 0 }}>
                {entry.score}
              </span>
            </div>
                  ...
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12 }}>
                      {a.isCorrect
                        ...
                        : <X     size={11} color="#EF4444" style={{ flexShrink: 0 }} />}
...
import { getAllPlayers } from "../utils/room.js";
  ...
  return (
    <ScreenWrapper withBg onExit={resetGame}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ textAlign: "center" }}>
          <h2 className="t-display" style={{ fontSize: 26, color: "#fff", marginBottom: 4 }}>
            Esperando al Líder...
          </h2>
          <p style={{ fontSize: 13, fontWeight: 700, color: "var(--c-w45)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {sorted.map((p, i) => (
              <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span className="t-display" style={{ fontSize: 20, color: "var(--c-gold)" }}>
