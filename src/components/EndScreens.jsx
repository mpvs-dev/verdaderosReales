    ...
    <ScreenWrapper withBg onExit={resetGame}>
      ...
      <div style={{ textAlign: "center" }}>
        <div
          ...
          style={{ fontSize: 48, display: "inline-block", marginBottom: 8 }}
        >
          🏆
        </div>
        <h1
          className="t-display"
          style={{ fontSize: 30, color: "#fff", marginBottom: 4 }}
        >
          {t("results.title")}
        </h1>
        {duration && (
          ...
          >
            ...
            <span
              style={{ fontSize: 12, fontWeight: 700, color: "var(--c-w45)" }}
            >
      ...
      {winner && (
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              {t("results.winnerSubtitle")}
            </div>
          </div>
          <div
            className="t-display"
            style={{ fontSize: 28, color: "var(--c-gold)", flexShrink: 0 }}
          >
            {winner.score} pts
          </div>
        </div>
        ...
        {sorted.map((entry, idx) => (
          <div key={entry.player?.id} className="glass">
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span
                className="t-display"
                style={{ fontSize: 20, color: "var(--c-gold)", flexShrink: 0 }}
              >
                {entry.score}
              </span>
            </div>
                ...
                {entry.answers.map((a, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className="truncate"
                        style={{ color: "var(--c-w45)", flex: 1 }}
                      >
                        {q?.text}:
                      </span>
                      <span
                        className="truncate"
                        style={{ fontWeight: 800, color: "var(--c-w60)" }}
                      >
                        {a.answer}
                      </span>
                    </div>
    ...
    <ScreenWrapper withBg onExit={resetGame}>
      ...
      >
        ...
        <div style={{ textAlign: "center" }}>
          <h2
            className="t-display"
            style={{ fontSize: 26, color: "#fff", marginBottom: 4 }}
          >
            {t("waitingQuestion.title")}
          </h2>
          <p style={{ fontSize: 13, fontWeight: 700, color: "var(--c-w45)" }}>
            <strong style={{ color: "#fff" }}>{currentRoom?.king?.name}</strong>{" "}
            {t("waitingQuestion.subtitle", {
              kingName: currentRoom?.king?.name,
            })}
          </p>
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {sorted.map((p, i) => (
              ...
              >
                ...
                <span
                  className="t-display"
                  style={{ fontSize: 20, color: "var(--c-gold)" }}
                >
