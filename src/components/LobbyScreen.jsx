...
export default function LobbyScreen({ roomCode, playerRole, currentRoom, startGame, gameConfig, setGameConfig, updateRoomConfig, resetGame }) {
  ...
  return (
    <ScreenWrapper withBg onExit={resetGame} exitLabel="Salir">
      {showConfig && (
      ...
      <div className="room-code-box">
        <div className="t-label" style={{ marginBottom: 4 }}>Código de sala</div>
      ...
      <div className="glass" style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="t-label">Administrador</div>
      ...
      {/* Config summary */}
      <div className="glass-gold" style={{ display: "flex", gap: 0 }}>
        {[
          { val: cfg.rounds,          label: "Rondas"     },
          { val: cfg.pointsPerAnswer, label: "Pts acierto" },
          { val: cfg.mode === "custom" ? "Custom" : "Genérico", label: "Modo", small: true },
        ].map(({ val, label, small }, i, arr) => (
          <div key={label} style={{
            flex: 1, textAlign: "center",
            borderRight: i < arr.length - 1 ? "1px solid var(--c-gold-border)" : "none",
          }}>
            <div className="t-display" style={{ fontSize: small ? 14 : 22, color: "var(--c-gold)", paddingTop: small ? 3 : 0 }}>
              {val}
            </div>
      ...
      <div className="glass">
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
          ...
          <span className="t-label">Jugadores ({aspirants.length})</span>
        </div>
            ...
            {aspirants.map((player, idx) => (
              <div key={player.id} className="anim-slide" style={{
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10, fontWeight: 700, color: "var(--c-w45)" }}>Jugador</div>
                </div>
              </div>
            ))}
          </div>
